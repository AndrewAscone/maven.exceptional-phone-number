...
public final class PhoneNumberFactory {
    ...
    public static PhoneNumber createPhoneNumberSafely(int areaCode, int centralOfficeCode, int phoneLineCode) throws InvalidPhoneNumberFormatException {
        String combinedNumber = "(" + areaCode + ")-" + centralOfficeCode + "-" + phoneLineCode;
        try {
            return createPhoneNumber(combinedNumber);
        } catch (InvalidPhoneNumberFormatException e){
            logger.warning(combinedNumber + " is not a valid phone number");
            return null;
        }
    }
